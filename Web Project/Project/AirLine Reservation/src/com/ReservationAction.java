    ...
    //   if(fnum.equals(Fnum)&& fname.equals(Fname))
       ...
       try{
    	   Class.forName("com.mysql.jdbc.Driver");
   		   ...
   		   System.out.println("connected");
           st=con.createStatement(); 
           int a=st.executeUpdate("insert into Reservation values('"+logForm.getCustid()+"','"+logForm.getFlightnum()+"','"+logForm.getFlightname()+"','"+logForm.getDeparture()+"','"+logForm.getArrival()+"','"+logForm.getOrig()+"','"+logForm.getDestination()+"','"+logForm.getSeats()+"') ");
           if (a>0)
           {
           System.out.println("inserted");
           }
