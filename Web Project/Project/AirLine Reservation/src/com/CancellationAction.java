		  ...
		  Statement  st=null;
         	try {
        	   ...
        	   Class.forName("com.mysql.jdbc.Driver");
             	...
             	int a=st.executeUpdate("insert into cancellation values('"+lg1Form.getCust_id()+"','"+lg1Form.getFlight_num()+"','"+lg1Form.getOrg()+"','"+lg1Form.getDestination()+"','"+lg1Form.getFlight_name()+"','"+lg1Form.getDep_time()+"','"+lg1Form.getArriv_time()+"','"+lg1Form.getSeat_num()+"')");
       		    if (a>0)
                    {
                System.out.println("Values are inserted");
                    }
       		    
             	rs=  st.executeQuery("select cust_id,flightnum,flightname,departure,arrival,orig,destination from reservation where  cust_id='"+C_id+"' and flightnum='"+Fl_num+"'and flightname='"+Fl_name+"'and departure='"+Dep_Time +"'and arrival='"+Arv_Time+"'and orig='"+Org+"'and destination='"+ Dest+"'");

       		while (rs.next())
       		{
       			 c_id =rs.getString("cust_id");
